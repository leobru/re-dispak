...
рчтбуф	пам	3
длягрп	конд	в'5000200000000004'   Е48,46,35
...
*
НАЧАЦП	уиа	'2003' <-НАЧАЛО,ОТРАБ2,НАЧDW,КОНЕЦ.
*Блокировка внешних прерываний
*
	сч	ШКАРУ  Шкала Работ
	и	Е48П37
	по	КОНЕЦ  -> ЕСЛИ В ШКАРУ.48-37 НЕТ РАБОТ
	нед	П77777 НОМЕР РАБОТЫ - 1
	уи	М6  уст на М6
	сч	ШКАРУ := погасил ЕДИНИЦУ АЦПУ
	нтж	E48(М6)
	зп	ШКАРУ := ПОГАСИЛ ЕДИНИЦУ АЦПУ В ШКАРУ
	счи	М6	НОМЕР АЦПУ ПЕЧАТИ
	сда	64-1 УМН. НА 2
	уи	М11  на М11 уст. удвоенный номер работы (0/2/4/…/22)
	сли	М11(М7) М11:=М11 + база драйвера.
	нтж	E2
	уи	М12  М12:=2/0/6/4…22/20,
*инверс ном работы – АЦП-0/1 (2/0)
*
	уии	М10(М6) М10=М6 (0/1/2/…/11). =0/1 ДЛЯ АЦП-0/1
	сли	М10(М7) М10=М10+М7.
*М10=СМЕЩЕНИЕ ЯЧЕЕК АЦП-0/1 В ЛИСТЕ ПЕЧАТЬ
*
	сч	0 ноль во всех 48 разрядах.
*Ниже верно при М6=0 или 1.
*
* ДАЛЕЕ РАБОТАЕТ КОРРЕКТНО ТОЛЬКО
*ДЛЯ ШКАРУ.48-37=Е48/Е47
*
...
*
	...
	сч	ШИФРАЦ Установим бит работы АЦПУ
*в шкале ШИФРАЦ.
*Е3 соотв. АЦПУ-1 И Е1 – АЦПУ-0 в ШИФРАЦ
*
	или	E3(М12) =Е3/Е1 для АЦПУ-1/АЦПУ-0 или Е1 при
*при М12=0/2
*
...
*:=Е25(М6) ПРИ ЕС-АЦПУ.
*
	сч	ВРЕМЯ  ЦЕНА МЛ. РАЗРЯДА ЯЧ ВРЕМЯ = 1/50 СЕК
*ВРЕМЯ = ЧИСЛУ ВРЕМ.ПРЕР-Й, ИДУЩИХ 50 РАЗ В СЕКУНДУ
*
	вч	рзвре1-П(М10) =128+ВРЕМЯ-Е24П1
	вчаб	E8 =256
	пе	ВКЛАЦП ->ЕСЛИ 128+ВРЕМЯ-Е24П1 < 0
	сч	шпр4к7(М6) ??Возможно шкала пропуска выдачи АЦП-0:1
	зп	шкприк-П(М10) :=ШПР4К7(М6) !!ШКАЛА ПРИКАЗОВ?
	пб	ВКЛАЦП       ->ВКЛАЦП    может хак
*НЕ ИСПользуется
	зп	МЕТОД-П(М10) НЕ ИСКЛЮЧАТЬ, ЧТОБЫ
	пб	ЗАНЯТО НЕ СДВИНУТЬ НИЖЕЛЕЖАЩИЙ  КОД
*
...
*ГАШ. 39Р.(38Р.) ГРП
*
	сч	шdw21а(М6)  <-ТВ67. шdw21а - это 2
*ячейки КОНФУС
*
	зп	NНПРDW-П(М10) ЗП. В ЯЧ. ИПП NНПРDW-П
	сч	E1 признак выделить память размером абзац
	пв	ТБУФ(М15) ->ТБУФ ФУНКЦ. ВЫД. ДЛЯ ПАМЯТИ БУФЕРА
* НОМЕР БУФЕРА НА СУММАТОРЕ НА СМ.7-1РР.
*
	сда	64-8 ->ТБУФ
	уи	М1  :=ФИЗ. АДРЕС ПЕРВОЙ вывод. ЯЧЕЙКИ БУФЕРА
...
*КОЛЬ ОБА АЦПУ НЕ ЗАНЯТЫ, ТО МОЖНО ИСПОЛЬЗ ИМЕЮЩИЙСЯ В ДРАЙВЕРЕ БУФЕР
*
...
чистка	уиа	'3' <-НЕИЗВ1,начис,зпупря. разрешить прерывание
*ДРАЙВЕРА ПЕЧАТЬ
*
	мода	(М15)
	пио	(М1) ->(М15), если нет выводного буфера
	уиа	-рзмбуф(М16) циклить 400 раз. ОБНУЛЕНИЕ БУФЕРА N1
...
*для АЦПУ0/АЦПУ1
	пб	(М15) ->чисвзв,чт1пз,ОЧСИМВ(2)
*.почист выв буф драйвера
*
отметь	зп	рчтбуф-П(М10) <-выдали, рчтбуф-П(М10):= 0
	сч	сектрк-П(М10)
	зп	упсктр-П(М10) сектрк-П(М10)
	слц	E1
	зп	сектрк-П(М10) сектрк-П(М10) + 1
чисвзв	уиа	-5(М4) <-отметь,СЧТЧИС. М4:=-5
*возврат после чистки выводного буфера
*
	уии	М2(М5) М2:=М5  адрес накопительного буфера
*драйвера
*
	сч	сектрк-П(М10) изначально сектрк-П(М10)=Е37
	и	E3
	по	чт1п3 -> ЕСЛИ НОМЕР СЕКТОРА ТРАКТА не= 4
	пб	отраб -> 4 СЕКТОРА ТРАКТА ОБРАБОТАНЫ.
*
*Чтение с 0 по 3-ий сектор тракта МБ.
чт1п3	уиа	0(М13)  <-ЧИСВЗВ. М13:=0 М13:=0
	сч	сектрк-П(М10)
	и	П3 номер сектора тракта добавили
*в КУС ФИЗОБМ с МБ
	или	шкусац-П(М10) ДЛЯ АЦП-0 / АЦП-1
	пв	ФИЗОБМ(М16) прочли очеред.сектор МБ
*в накопитель.буферa
	сч	сектрк-П(М10)
	и	П3
	пе	ОЧСИМВ -> ЕСЛИ не 0-Й сек ТРАКТА МБ АЦПУ0/АЦПУ1
	сч	(М5) =0-Я ЯЧ 0-ГО СЕКТОРА ТРАКТА ДЛЯ АЦП-0 / АЦП-1
	и	E31 Выяснить. РИМ - РОБОТ.
	по	бвывdw  если 31р. 0-ЯЧ 0-ГО сектор МБ = 0
*(в М5 адрес абзаца)
*
	сч	, =0
	зп	счзнбф-П(М10) :=0
...
ОЧСИМВ	ноп	,
	...
	пв	(М16)  ->ВЫБСИМ
	уиа	'3'    <-ВЫБСИМ
	по	КОНЗОН -> ЕСЛИ СИМВОЛ = 0
	нтж	П177
	по	ПОВТОР ->ПОВТОР, ЕСЛИ СИМВОЛ = 177(8)
*переход на новую строку
	...
	пб	ОЧСИМВ ->
***
неисп1	сч	сектрк-П(М10) <-!!СЮДА НЕТ ПЕРЕХОДА В ЭТОМ
*МОДУЛЕ
*
	или	E48
	зп	сектрк-П(М10) :=Е48 + СЕКТРК-П(М10)
	сч	ШКАРУ
	и	E24(М6)
	нтж	ШКАРУ
	зп	ШКАРУ :=ШКАРУ БЕЗ Е24(М6)
	сч
	зп	ряес2-П(М10) :=0
	пб	встат
***
...
***
пркдdw	сч	шdw21б <-ОЧСИМ1. ПРОВ. ГОТОВНОСТИ DW-21
	и	е17dw(М6) NKAН ИНДЕКСИРУЕТСЯ ПО М6
	по	(М17) ->ПРКDW1, ЕСЛИ НЕ АЦП-0
	мода	пркод7
	пио	(М6)  если ацпу-0, то -> конфус.пркод7
	сч	cимвdw-П(М10) !! СЮДА НЕ ПРИДЕМ
	и	П17
	уи	М16 М16:=СИМВDW-П(М10).4-1РР
	нтж	cимвdw-П(М10) СМ.8-5,4-1=СИМВDW-П(М10).8-5,0
	сда	64+1
	уи	М3 М3.7-4,3-1:= СИМВDW-П(М10).8-5,0
	мода	(М16)
	сч	ТАБЛИЦ+5 (М16). ЯЧЕЙКИ: ТАБЛИЦ.5-24
	сда	64(М3) СДВИГ ВПРАВО НА 40,32,24,16,8,0 РР
*М3 := 0..7?
*
	и	П377 ПЕРЕКОДИРОВАЛИ СИМВОЛ
	зп	cимвdw-П(М10) ПЕРЕКОДИРОВАЛИ СИМВОЛ
	пб	(М17) >ПРКDW1
***
очсим1	сч	символ-П(М10) <-ОЧСИМВ
	вч	E1
	зп	cимвdw-П(М10) :=символ-П(М10) - 1
	вч	П140 =96(10) максимальное кол-во
*строк в буфере = 96 строк * 128 симв.
*
	по	выдали ЕСЛИ символ-П(М10) >= 141(8)
*записать шкалы перед обнулением
*буфера.
*
	мода	пркдdw ->ЕСЛИ СИМВDW-П(М10) >= 140(8)
	пв	(М17) ->пркдdw СИМВDW-П(М10) =< 137(8)
***
пркdw1	сч	cимвdw-П(М10) <-пркдdw,ОЧСИМ1
...
*
	...
	вч	П60  =м17-48(10)
	пе	пркdw2 -> ЕСЛИ СИМВDW-П(М10) < 60(8)
	слиа	1(М16) М16:=А(ШКУСАЦ+3/ШКУСАЦ+5)
*ДЛЯ АЦП-0/1 ПРИ СИМВDW-П(М10) >= 60(8)
*
	уи	М17  М17:=М17-48. ИНДЕКСИРУЮТСЯ СТРОКИ,
*ВЫД. НА АЦПУ.
*
пркdw2	сч	E48(М17) <-ПРКDW1. НОМЕР СТРОКИ.
*НА М16 НА 1 БОЛЬШЕ
*
	или	(М16) при  символ >=48,
*ДЛЯ АЦПУ-0 М16=ШКУСАЦ+2/+3 ПРИ СИМВDW+0=0-47/48-95
*ДЛЯ АЦПУ-1 М16=ШКУСАЦ+4/+5 ПРИ СИМВDW+1=0-47/48-95
*
	зп	(М16)
	сч	cимвdw-П(М10)
	и	П37 выделили младшие 5рр. символа
	сда	64-3 симвdw*8
	уи	М3  М3:=СИМВDW-П(М10).5-1*8.
*УСТ. ЧАСТЬ СИМВDW НА М3
*
	сли	М3(М1) М3:=М3+М1. НА М1 АДРЕС
*АБЗАЦ БУФЕРА ВЫВОДА НА АЦП
*
	сч	cимвdw-П(М10) =<137(8)
	сда	64+5 =0/1/2
	вчоб	E2 =2/1/0
	сда	64-4 =40/20/0. СИМВОЛ.7-6 * 16
	зп	cимвdw-П(М10) :=40/20/0
*ЗАВИСИМО ОТ СИМВDW-П(М10).7-6
*!!НЕ ПОНЯЛ ИСП-Е ТАКОГО СИМВDW-П(М10) В ЧТОБЩ1
*
	пб	чтспец
ПОВТОР	мода	ВЫБСИМ <-ОЧСИМВ
	пв	(М16) ->ВЫБСИМ
дальше	вч	E1 <-ПОВТОР,ЧТОБЩ1.  =СИМВОЛ-1
	зп	СИ-П(М10) :=СИМВОЛ-1
чтспец	мода	чтобщ <-ПРКDW2,ДАЛЬШЕ
	пио	(М6) ->ЧТОБЩ, ЕСЛИ М6=0 (Т.Е. АЦП-0)
*
	сч	шdw21б РАБОТА с АЦПУ DW-21
	и	е17dw(М6) ПРОВЕРКА NКАН DW-21
	по	чтобщ если ацпу-128, А НЕ DW-21
	счи	М13 М13=СИМВОЛ. ПРИНТЕР DW21 НА КАНАЛЕ АЦП
	сда	64+4 =7-5РР СИМВОЛА
	уи	М16 М16:=М13 / 16  (М16=0/1/2/3/4/5)
	сли	М16(М3) М16:=М16+М3
*(М3=[СИМВDW-П(М10).5-1] * 8 + М1)
*
	счи	М13
	и	П17 =4-1РР СИМВОЛА
	пб	чтобщ1 ->
...
*
чтобщ	счи	М13 <-ЧТСПЕЦ(2). М13=7-1рр. яч. символ
	и	П7 =3 младших разряда
	уи	М16
*Если АЦПУ-128, то М3 не опред.
	сли	М16(М3) М16:= 3 МЛ.РР СИМВОЛ +
*СМЕЩ В БУФЕРЕ ВЫВОДА
*
	счи	М13
	сда	64+3
чтобщ1	нтж	П17 4 младших разряда
	слц	cимвdw-П(М10)
	уи	М17 М17:=СИМВDW-П(М10) И
*ИНВЕРСИЯ 1-4РР СИМВОЛА
	сч	E48(М17)
	или	(М16)
	зп	(М16)
	слиа	1(М13) М13:=М13+1
	сч	СИ-П(М10)
...
*ДЛЯ АЦПУ0/1
	сда	64-15 СМ.22-16РР:= ЧЕД(ШКУСАЦ+2-3/4-5),0+
*ЧЕД(ШКУСАЦ+2/4)
...
отраб1	уиа	11(М15) <-ОТРАБ. М15:=13(8)
	счи	М14 СМ:== М14 ДЛЯ УПРЯТ В СТЕК АЦП-0/АЦП-1
отраб2	слиа	-1(М15) ->ОТРАБ2,М15:=М15 - 1
	счим	М1(М15) УПРЯТ ОЧЕРЕД М14-М2 В СТЕК АЦП-0/АЦП-1
	мода	отраб2
	пино	(М15) ->G10250, ЕСЛИ М15 НЕ= 0
	зп	(М17) УПРЯТ М1 В СТЕК+13(8),
*М17:=А(СТЕК)+14(8)
	сч	ВРЕМЯ
	зп	(М17) :=ВРЕМЯ В А(СТЕК)*13, М17:=А(СТЕК)*14
	пб	НАЧАЦП
*
пропус	сч	шкприк <-КОНЕЦ копия шкалы
* пропуска выдачи
*
	сл	D10674
	зп	D10675
	сч	E7 заказанное время
	пв	ТАЙМЕР(М15) завели будильник
	пв	ТБУФ13(М16) через дисп70. на ВИСП3
	уиа	'2003'
	сч	ШКАРУ
	и	Е24П13
	по	G10273
...
G10273	сч	ШКАРУ
	сда	64-12
	и	Е48П37
	пе	сохран ->сохран, ЕСЛИ ШКАРУ.36-25РР >0
*уходим сюда
	сч	ШКАРУ
	сда	64-24
	и	Е48П37
G10277	по	G10277 <-G10273,<->G10277,
*ЕСЛИ ШКАРУ.24-13РР=0 ??
*
	нед	П77777
	уи	М6
	сч	ШКАРУ
	нтж	E24(М6)
	зп	ШКАРУ
G10302	уиа	-11(М16) <-G10277,<-сохран. М16:=-13(8)
	мода	упрят0+12 ПОСЛЕД ЯЧ ПОЛЯ УПРЯТ
*М14-М1 И ВРЕМЯ ДЛЯ АЦП-0
*
	уиа	0(М17)
	счи	М6
	по	встрег -> ЕСЛИ АЦП-0
	слиа	13(М17) М17:=А(УПРЯТ0)+31(8) – АДР
*УПРЯТ ЯЧ ВРЕМЯ ДЛЯ АЦП-1
*
***Восстание упрят. значения М1-М14
*
встрег	уиа	'3'  <-G10302(2), РАЗРЕШИТЬ ВНЕШН ПРЕРЫВАНИЯ
	сч	(М17) =УПРЯТАННОЕ ЗНАЧЕНИЕ М1 ДЛЯ АЦП-0/1
встцкл	уим	М14(М16) <-встрег,встцкл. ВОССТ М1-М14 ИЗ СТЕКА
	мода	встцкл
	цикл	(М16) ->встцкл, ЕСЛИ М16=<0, М16:=М16+1
встат	пам	0  <-встцкл,G10151
	сч	сектрк-П(М10)
	рзб	РЗБ111
	слц	СТУСТР+7
	зп	СТУСТР+7
	сч	сектрк-П(М10)
	и	E47
	по	гасгрп ->гасгрп, ЕСЛИ сектрк-П(М10).47-47РР=0
	нтж	сектрк-П(М10)
	зп	сектрк-П(М10) :=СЕКТРК-П(М10) – Е47
	сч	упрбф2-П(М10) СОДЕРЖИТ УПРЯТАННОЕ
*М2 СОГЛАСНО зпупря
*
	пе	встбуф ->встбуф, ЕСЛИ сектрк-П(М10) не = 0.
	уии	М2(М5) М2:=М5
	уиа	-5(М4) М4:=-5
	пб	G10321 ->G10321
*
встбуф	уи	М2 <-встат, ВОССТАНОВЛЕНИЕ м2
	мод	упрм4а-П(М10)
	уиа	0(М4) М4:=упрм4а-П(М10)
G10321	уиа	0(М13) <-встат,встбуф. М13:=0
	сч	E1
	зп	рчтбуф-П(М10) :=Е1
	сч	усктр1-П(М10)
	нтж	сектрк-П(М10)
...
начис	сч	усктр1-П(М10)
	и	П3
	или	E37
	зп	сектрк-П(М10) := Е37+усктр1-П(М10).2-1
	сли	М15(М7) M15:=А(ПЕЧАТЬ)+А(чт1пз-П)/A(ОЧСИМВ-П)
	пб	чистка ->ЧИСТКА, ОТКУДА ПЕРЕЙДеТ ПО М15
*
	сч	E1              зачем это
*
*
***Блок работы после выдачи содержимого
***буфера размером 96 строк * 128 симв.
*
выдали	зп	рчтбуф-П(М10) <-G10167.
*рчтбуф-П(М10):=D10703-П(М10) - 141(8)
*
	нтж	П37
	нтж	E3
	по	отметь -> ЕСЛИ рчтбуф-П(М10)=33(8)
	нтж	П6
	по	отмп6 -> ЕСЛИ рчтбуф-П(М10)=35(8)
	нтж	E1
	пе	отраб  -> ЕСЛИ рчтбуф-П(М10)=34(8)
	уиа	'3'
	уиа	0(М2) М2:=0 ЧИСЛО ЕДИНИЦ РАЗДЕЛА ЧАСЫ
	уиа	-2(М4) М4:=-2 НАЧНЕМ С ПОДСЧЕТА
*ЕДИНИЦ РАЗДЕЛА ЧАСЫ
*
	сч	ВРЕМЯ =ЧИСЛО ВРЕМЕННЫХ ПРЕРЫВАНИЙ
*(В ТЕЧЕНИИ СУТОК), ИДУЩИХ 50 РАЗ В СЕК
*
времпр	зп	ряучет <-выдали,времпр,
*РЯУЧDW:=ОСТАТОК ВРЕМЕНИ
	мода	час
	вч	2(М4) М4=-2,-1,0 – ТЕКУЩИЙ РАЗДЕЛ,
*СОДЕРЖИТ ЧИСЛО ВРЕМ. ПРЕРЫВАНИЙ.
	пе	пчтном печать номера эвм,
*ацпу, даты и времени.
*ЕСЛИ РЯУЧDW < ЗНАЧЕНИЯ ТЕКУЩЕГО РАЗДЕЛА
*
	слиа	1(М2) М2:=М2+1 УВЕЛИЧИЛИ СЧЕТЧИК
*ТЕКУЩЕГО РАЗДЕЛА
	пб	времпр
*
сохран	нед	П77777 <-G10273,КОНЕЦ. УСТАНОВКА
*NКАН АЦПУ. =ПОЛЕ
	уи	М6  =0/1 НОМЕР АЦП-0/1
	сч	ШКАРУ
	нтж	Е36(М6)
	зп	ШКАРУ :=ШКАРУ-Е36/Е35 ДЛЯ АЦП-0/1
	пб	G10302 ->
*
*** Блок воврата занятых ресурсов
*
гасгрп	сч	сектрк-П(М10)
	и	длягрп
	по	счзон
	сч	E48(М6)
	или	E39(М6)
	...
	уиа	'3'
	счи	М1
	по	верни1 если был взят буфер ранее?
	сда	64-2
	пв	ТБУФ(М15) вернули
* на сумматоре только  Е1
*
верни1	сч	кусвыв-П(М10)
	по	верни2
	пв	ТБУФ(М15) вернули лист
	сч
	зп	кусвыв-П(М10)
верни2	сч	занац0-П(М10)
	пе	запиши
	сч	шкусац-П(М10)
	пв	ТБУФ(М15)
*
***ЗАПИСЬ В ШКАЛАХ ОБ ОКОНЧ. ВЫДАЧИ НА АЦПУ
*
запиши	сч
	зп	занац0-П(М10)
	сч	E3
...
КОНЕЦ	уиа	'2003' <-НАЧАЦП,cтацпу.
	...
	сда	64-12
	и	Е48П37
	пе	сохран -> ЕСЛИ ШКАРУ.36-25 не = 0
	сч	ШКАРУ
	и	Е48П37
	пе	НАЧАЦП -> ЕСЛИ ШКАРУ.48-37 не = 0
	сч	ШКАРУ ШКАРУ.48-37 = 0
	и	Е24П13
	пе	пропус -> ЕСЛИ ШКАРУ.24-13 не = 0
	пв	ЗАНОСМ(М16) ШКАРУ 24-13рр. = 0. УЧТЕМ
*ВРЕМЯ РАБОТЫ ОС
*
	сч	E47  РАЗРЯД ПРОЦЕССА ПЕЧАТЬ (Е47 В ШГ)
	пб	ОТКАЗН ОБРАБОТКА КОНЦА НЕРЕЗИДЕНТНОГО ПРОЦЕССА
*
...
ВЫДОПТ	уии	М32(М15)
	...
	пб	ТВ40
***
ВЫБСИМ	сч	2(М2) <-ОЧСИМВ,ПОВТОР. СЛОВО ТЕКСТА
	мода	5(М4)
	сд	ДВБАЙТ СДВИГ ВПРАВО НА 40,32,24,16,8,0
*ПРИ М4=-5,-4,-3,-2,-1,0
*
	и	П377
	зп	символ-П(М10) :=48-9,8-1РР=0,СИМВОЛ
	мода	(М16)
	цикл	(М4) ->ОЧСИМВ,ДАЛЬШЕ, ЕСЛИ М4:=М4+1 =< 0
	слиа	1(М2) М2:= СЛЕД СЛОВО ТЕКСТА
	уиа	-5(М4) М4:=-5 – ВЫБИРАТЬ САМЫЙ ЛЕВЫЙ
*БАЙТ СЛЕД СЛОВА
*
	пб	(М16) ->ОЧСИМВ,ДАЛЬШЕ
        отмен   (М7)
*
********************
...
СИАЦПУ	слиа	-1(М16)
	...
	увв	'4031'
	и	E24(М3)
	по	протяж          идет протяжка бумаги
	сч	сектрк-П(М16)
	или	КСД3
	нтж	КСД3
	зп	сектрк-П(М16)
...
рфизоб	пам	1
*
*** ПЕЧАТЬ ИНФ. О ВРЕМЕНИ РЕШ. И ПЕЧАТИ
*** НА ЛИСТАХ, ВЫДАВАЕМЫХ НА АЦПУ DW-21
*
...
*
тучdw1	конд	в'5560040100200674'
тучdw2	конд	м40в'275'м32в'042'м24в'014'м16в'001'м8в'301'в'302'
часмин	конд	п'11 11 '
тучdw3	конд	в'0020071363000401'
тучdw4	конд	п'.00.00'
...
учбуф1	сч	E1 <-учтбуф
	пб	учтбуф
*
учтdw1	сда	64-15 <-учетdw. 30-16РР=-5 . М4
	...
	зп	тучdw2 := КАКОЙ-ТО ТЕКСТ С NБЭСМ
	сли	М16(М14) М16:= NБЭСМ + АДР ЛИСТА КАТ БУФ ВЫВОДА
	сч	'1761'(М16) ЛИСТ КАТАЛОГА ВЫВОДА
	слц	E1 +1
учтбуф	зп	'1761'(М16) <-учбуф1 НА СМ Е1.
*’1761’(М16):=Е1/’1761’(М16)+Е1
*
	зп	D10035-П(М10) D10035-П(М10):= ’1761’(М16)
	и	E13
	пе	учбуф1 -> ЕСЛИ ’1761’(М16).13Р НЕ= 0
*(ЗНАЧЕНИЕ ’1761’(М16) >= 4096)
*
	сч	'1761'(М16)
	...
	зп	тучdw1 :=КАКОЙ-ТО ТЕКСТ 4-МЯ ЦИФРАМИ
	сч	ВРЕМЯ ВРЕМЯ ИЗ ДИСП70. 1 ТИК = 1/50 СЕК
учвре	зп	ряучет <-учвре,учвре1. ОСТАТОК ВРЕМЕНИ (Т)
	...
	пб	учвре -> ПРОДОЛЖ.ПОДСЧЕТ ТЕКУЩ ТИПА Т
учвре1	сч	типтик <-учвре. М2=ЗНАЧ-Е ТЕКУЩ ТИПА Т
	сда	64-16
	...
	мода	учвре
	цикл	(М4) ->учвре, М4:=М4+1 (М4=-1 И М4=0)
	сч	типтик учвре=6 ЦИФР: 2 цифры-ЧАС,
*2-МИНУТА,2-СЕКУНДА
*
	...
	сч	типтик
	сда	64-32
	слц	прбсек 2 ЦИФРЫ=+1(СЕКУНДА), +2 СПЕЦ СИМВ
*сдвиг на 76 позицию строки
	зп	тучdw3 := 2 СИМВ СЕКУНДА, 2 СПЕЦ СИМВ
	сч	ГОД !! УЗНАТЬ РАЗРЯДЫ ЯЧЕЙКИ ГОД
	сда	64-13 35-1РР ГОД СДИНУЛИ В 48-14РР
	рзб	рбтdw1 дни даты
	слц	тучdw3
	зп	тучdw3 := ДОБАВИЛИ ЧТО-ТО?? ИЗ ГОД В тучdw3
	сч	ГОД
	сда	64-19 29-29РР,28-25РР,24-21РР ГОД СДИНУЛИ В
	рзб	рбтdw2 месяц + цифры десятков/единиц года
*33-33РР, 28-25РР, 4-1РР
*
	слц	D11240 + .. В 48-41 И 24-17РР, + 0 В 16-9РР
	зп	тучdw4 := ДОБАВИЛИ 3 ЦИФРЫ ИЗ 29-21РР
*ГОД В тучdw4
*
	сч	ГОД
	сда	64+16
	и	П17 20-17РР ЕЩе ОДНА ЦИФРУ ГОД
	сда	64-8 СДВИНУЛИ В 12-9РР
	слц	тучdw4 + К ТРеМ ЦИФРАМ ГОД В ЯЧ тучdw4
	слц	ЕДИНИЦ 6 СИМВОЛОВ УВЕЛИЧИЛИ НА 1
	зп	тучdw4 := 4 ЦИФРЫ ИЗ 29-17РР ГОД В тучdw4
	уии	М2(М14) М2:= АДРЕС ЛИСТА С КАТАЛОГОМ ВЫВОДА
	уиа	-14(М16) М16:= ЦИКЛ ПОВТОРЯТЬ 15 РАЗ
пердрв	мода	14(М16) <-учвре1,пердрв.
*ПЕРЕПИСЬ 0-14 ЯЧ. ДРАЙВЕРА АЦПУ
*
	сч	учетdw С яч. учетdw ПО яч. учтdw1
	мода	14(М16) В ПЕРВЫЕ 15 ЯЧЕЕК
	зп	(М2)  ЛИСТА ??С КАТ. ВЫВОДА
	мода	пердрв ->пердрв. КОНЕЦ ЦИКЛА ПЕРЕПИСИ
	цикл	(М16) КОНЕЦ ЦИКЛА ПЕРЕПИСИ
	уиа	-5(М4) М4:-5
	сч	кусвыв-П(М10) КУС КАТАЛОГА БУФЕРА ВЫВОДА
	пв	DЕQ(М15) ОСВОБОДИЛИ ДИСК КАТАЛОГА БУФЕРА ВЫВОДА
	пб	прибав
*
непуст	сч	(М14) КОНЕЦ ЦИКЛА ПЕРЕПИСИ
	по	бфдисw -> ЕСЛИ 0-Я ЯЧ. ЛИСТА = 0
	сч	,
	зп	(М14) :=0
	уии	М2(М14) М2:=М14 – АДРЕС ЛиСТА
	слиа	1(М2) М2:=М2+1 – АДРЕС ЯЯ.+1 ЛИСТА
	уиа	-5(М4) НОМЕР БАЙТА
	пб	зпупря ->
*
*
бфдисw	сч	абуфdw-П(М10) <-НЕПУСТ
	уи	М2 М2:=АБУФDW-П(М10).15-1РР – АДРЕС ЛИСТА
	сда	64+15
	уи	М4 М4:=АБУФDW-П(М10).30-16РР
	сч	кусвыв-П(М10)
	пв	ЕNQ(М15) ЗАХВАТ ДИСКА С КАТАЛОМ ВЫВОДА
	сч	кусвыв-П(М10)
	пв	ФИЗОБМ(М16) ЧТЕНИЕ КАТАЛОГА ВЫВОДА В ЛИСТ
	сч	ГОД
	и	П7
	уи	М16 М16:=ГОД.3-1РР. НОМЕР БЭСМ-6 ПРЕДПРИЯТИЯ
	сли	М16(М14) М16:=М16+М14.
*АДРЕС В КАТАЛОГЕ + НОМЕР БЭСМ-6
*
	сч	D10035-П(М10) =УПРЯТАННОЕ ЗНАЧЕНИЕ ’1761’(М16)
	зп	'1761'(М16) :=ВОССТАНОВИЛИ В ЛИСТЕ КАТОГА ВЫВОДА
	сч	кусвыв-П(М10)
	пв	ФИЗЗП(М16) ЗАПИСАЛИ ИЗМЕНЕННЫЙ КАТАЛОГ
*ВЫВОДА НА ДИСК
*
	сч	кусвыв-П(М10)
	пв	DЕQ(М15)
	сч	кусвыв-П(М10)
	пв	ТБУФ(М15) ОСВОБОДИЛИ ЗАПРОШЕННЫЙ
*ЛИСТ ПОД КАТАЛОГ ВЫВОДa
	сч
	зп	кусвыв-П(М10) :=0
зпупря	мода	ОЧСИМВ <-счзон,прибав,заверш,непуст,бфдисw
...
кусвыв	пам	2
* ДЛЯ РАЗБОРКИ СООБЩ. ВЫД. НА АЦПУ DW-21
D11235	конд	м40в'275'м32в'042'м24в'014'м16в'001'м8в'301'в'302'
D11236	конд	п'011110'м40в'267'в'274' преобр. симв. в формат
* буфера вывода.
прбсек	конд	п'110011'м24в'313'м16в'314'
D11240	конд	п'.00.00'
*
занац0	пам	1
занац1	пам	1
